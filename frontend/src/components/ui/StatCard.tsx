...
export default function StatCard({
  ...
  return (
    <div className={`bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/90 shadow-2xs hover:shadow-xs transition flex flex-col justify-between ${className}`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {badge}
          {icon && (
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border border-slate-100 ${iconBg}`}>
              {icon}
            </div>
          )}
        </div>
